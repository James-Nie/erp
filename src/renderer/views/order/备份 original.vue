  ...
  <div class="order-management">
    <div class="mgb15">
      <el-dropdown @command="handleCommand">
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-for="item in commands" :key="item.shopUuid" :command="item.shopName">{{item.shopName}}</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
      ...
      <el-dropdown class="mgl10" @command="selectedCommand">
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-for="item in colorOptions" :key="item.font_color_uuid" :command="{font_color_uuid:item.font_color_uuid, name: item.name}">{{item.name}}</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    ...
    <div class="table-wrapper">
      <el-table :data="orderList" v-loading="loading" ref="orderList" style="width: 100%" highlight-current-row @current-change="selected" @selection-change="handleSelectionChange" max-height="523" :row-style="setRowStyle">
        ...
        <el-table-column label="发货状态" :formatter="$formatter.formatOrderStatus" align="center">
        </el-table-column>
        <el-table-column prop="orderType" label="订单分类" align="center">
        </el-table-column>
        <el-table-column label="退款状态" :formatter="$formatter.formatRefundStatus" align="center">
        </el-table-column>
    ...
    <el-dialog size="tiny" custom-class="cancel-reason" title="取消原因" :visible.sync="cancelReasonVisible" @close="closeCancel">
      <cancel-reason :orderNumber="current ? current.orderNumber : ''" ref="cancel"></cancel-reason>
    </el-dialog>
  ...
  methods: {
    ...
    querySupplier() {
      this.$tool.loadOptions(this, 'supplier/list', bodyInfo => {
        this.supplierList = bodyInfo.supplierList.map(item => ({value: item.name}))
      })
    },
    // 导出Excel
    exportExcel() {
    ...
    frozenOrder() {
      ...
      if (!status[this.current.orderStatus]) {
        this.$message({
    ...
    handlePopupForm(formName) {
      if (formName === 'add') {
        ...
        let statusNumber = this.current.orderStatus
    ...
    queryShop() {
      this.$tool.loadOptions(this, 'store/list', bodyInfo => {
        this.commands = bodyInfo.ShopList
      })
    },
    // 打开字体颜色管理面板
    openFontColorPanel() {
    ...
    queryFontColor() {
      this.$tool.loadOptions(this, 'fontColor/list', bodyInfo => {
        this.fontColorData = bodyInfo.FontColorList.map(this.filterFontColor)
      })
    },
    // 查询字体颜色下拉列表
    queryFontColorDis() {
      this.$tool.loadOptions(this, 'fontColor/listDis', bodyInfo => {
        this.colorOptions = bodyInfo.FontColorDisList.map(this.filterFontColor)
      })
    },
    // 新建字体颜色
    addFontColor() {
    ...
    updateFontColor(row, event, column) {
      ...
      this.fontColorData.forEach(item => {
        if (item.font_color_uuid === row.font_color_uuid) {
          item.editable = true
      ...
      if (!this.fontColorWriteable) {
        if (row.font_color_uuid !== this.fontColorData[this.fontColorData.length - 1].font_color_uuid) {
    ...
    saveFontColor() {
      ...
      if (this.saveFontColorType === 'add') {
        ...
        this.$tool.common(this, 'fontColor/update', editItem, bodyInfo => {
          this.queryFontColor()
          this.queryFontColorDis()
          this.fontColorVisible = false
        })
    ...
    selectedCommand(command) {
      let orderNumbers = this.multipleSelection.map(item => item.orderNumber)
      ...
      this.$tool.common(this, 'order/fontColor', {
        ...
        font_color_uuid: command.font_color_uuid
      }, bodyInfo => {
        this._loadMainList()
      })
    },
    ...
    // 查询车间列表
    queryWorkshop() {
      this.$tool.loadOptions(this, 'workshop/selete', bodyInfo => {
